...
async function distribuir(linhaId, vendedoraId, gerenteTelegramId) {
  return db.transaction(async trx => {
    ...
    const resultadoEnvio = await trx('lead_envios').insert({
      nome: `Futuro cliente - ${nomeEmpresa}`.slice(0, 240),
      total_linhas: 1,
...
async function processarCallback(query) {
  ...
  if (acao === 'atribuir') {
    ...
    await telegramService.chamarApi('editMessageReplyMarkup', { chat_id: query.message.chat.id, message_id: query.message.message_id, reply_markup: { inline_keyboard: [] } });
...
module.exports = { receberWebhook, processarCallback, distribuir };
